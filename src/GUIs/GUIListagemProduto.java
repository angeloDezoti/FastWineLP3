...
public class GUIListagemProduto extends JDialog {
    ...
    public GUIListagemProduto(List<Produto> texto) {
        ...
        String[] colunas = new String[]{"IdProduto", "NomeProduto", "PaisDeOrigem", "TeorAlcoolico", "VolumeProduto", "PrecoProduto", "IdTipo", "CaminhoImagem",};
        String[][] dados = new String[0][3];
        DefaultTableModel model = new DefaultTableModel(dados, colunas);
        JTable tabela = new JTable(model);
        scroll.setViewportView(tabela);
        for (int i = 0; i < texto.size(); i++) {
            String[] linha = new String[]{String.valueOf(texto.get(i).getIdProduto()), String.valueOf(texto.get(i).getNomeProduto()), String.valueOf(texto.get(i).getPaisDeOrigemProduto()), String.valueOf(texto.get(i).getTeorAlcoolicoProduto()), String.valueOf(texto.get(i).getVolumeProduto()), String.valueOf(texto.get(i).getPrecoProduto()), String.valueOf(texto.get(i).getTipoVinhoidtipo()), String.valueOf(texto.get(i).getCaminhoImagemProduto()),};
            model.addRow(linha);
        }
        painelTa.add(scroll);
